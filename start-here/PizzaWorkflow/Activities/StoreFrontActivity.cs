...
public class StorefrontActivity : WorkflowActivity<Order, Order>
{
    ...
    public StorefrontActivity(DaprClient daprClient, ILogger<DeliveryActivity> logger)
    {
    ...
    public override async Task<Order> RunAsync(WorkflowActivityContext context, Order order)
    {
        try
        {
            ...
            _logger.LogInformation("Order {OrderId} processed with status {Status}", order.OrderId, response.Status);
